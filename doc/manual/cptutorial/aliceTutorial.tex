...
\markboth{\it {\bf Constraint Programming in Alice. A tutorial.} - Dominik
  Brill}
...
file: 
\htmladdnormallink{Alice sources}{http://www.ps.uni-sb.de/~brill/fopra/Beispielprogramme}
%-------------
% comes later...
%------------
...
\htmladdnormallink{\cite{1} {\it Finite Domain Constraint Programming in Oz. 
A Tutorial.} \\
by {\it Christian Schulte and Gert Smolka}}
...
\{1, $\ldots$, 81\}.
\end{enumerate}
\db{In the first viewpoint you only need the propagator {\it distinct(s,v)}
that is already implemented in the Alice library and that ensures
that all elements of the vector {\it v} are distinct in space
{\it s}, i.e. pairwise non-equal}. 
...
CSP. \db{One way is by {\it off-line search}
...
\lstset{caption = {\bf A script for the Send More Money
...
the script the structures {\it FD}, {\it Linear} and {\it Explorer}
...
\htmladdnormallink{Alice sources}{http://www.ps.uni-sb.de/~brill/Beispielprogramme}.}}
The script first declares a vector of finite domain variables,one for every letter. 
This is done via
the function {\it fdtermVec: space * int * domain $ \rightarrow $ term vector}
included in the structure
\htmladdnormallink{\textcolor{blue}{Linear}}
{http://www.ps.uni-sb.de/alice/manual/library/linear.html} from the Gecode 
library. \db{ The function {\it fdtermVec(s,i,d)} creates a 
new vector of finite domain variables 
with lenght {\it i}
and every variable has to range over a finte 
set of integers specified by
{\it d} ( so it is a basic constraint)}. 
Then the script posts the
following constraints:
\begin{itemize}
\item
The fields of the vector {\it letters} are pairwise 
distinct. This is
realized with the predefined propagator \\
{\it distinct : space * term vector * conlevel $ \rightarrow $ 
unit}, also included
in the Linear library. {\it Conlevel} refers to 
the strength of
propagation, i.e. for most propagators you can select 
{\it interval propagation} (FD.BND)
or {\it domain propagation} (FD.DOM). 
This constraint 
is nonbasic.
\item
...
\caption{{\bf The Explorer with the compact search tree of smm.}}
\label{figure5}
\end{figure}
...
\db{Select the red triangle and type the command {\it H} (hide/unhide). 
...
the command {\it H}. If you hide a subtree with command {\it H} and
...
{\it F} just hides failed subtrees.}
\begin{figure}[htpb]
\centerline{\includegraphics*[scale=0.5]{figs/smm2.eps}}
\caption{{\bf The Explorer with the full search tree of smm}}
\label{figure6}
\end{figure}
...
\db{A new window is popped - up by the {\it Alice Inspector}.
...
\db{You can stop the exploring Explorer at any time with the command {\it Ctrl + C}}.

\subsubsection{Resuming exploration}
\db{You can resume the exploration of a partial search tree by selecting 
any choice node and typing the command {\it N} or {\it A}}. The command {\it N} (next) 
...
\db{The command {\it Ctrl + R} will reset the Explorer and show only the root node of 
the search tree (grey triangle). You 
can request the exploration of the seach tree by typing {\it N} or {\it A}}.

\subsubsection{Hand-guided exploration}
\db{You can guide the search of the Explorer by hand. Reset the Explorer 
by typing {\it Ctrl + R}. This will select the root node, which is an open choice 
node. Now type the command {\it S} to compute the first descendents of 
...
\subsubsection{Problem Specification}
The code of Professor Smart's safe is a sequence of 9 distinct nonzero digits 
$ C_1,\ldots,C_9 $ such that the following equations and inequations are satisfied:
   $ \begin{array}{c} C_4 - C_6 = C_7 \\ 
         C_1 * C_2 * C_3 = C_8 + C_9 \\ 
                C_2 + C_3 + C_6 < C_8 \\ 
                             C_9 < C_8 \\ 
           C_1\neq 1,\ldots, C_9\neq 9 
...
first-fail strategy. This time, the domains of the variables range over
\{1, $\ldots$, 9\}.
...
\lstset{caption = {\bf A script for the Safe Puzzle},label=listing 2,frame=trbl}
\begin{myverbatim}
fun safe space = 
   let
...
First, the script introduces a vector of type {\it intvar}, with one 
...
part of the {\it let -  construct}, the first constraint ensures that all digits
...
you recognize a {\it FD} with the finite domain variables. This is important because the {\it
post} method gets besides the others one argument of type {\it term}. {\it FD} transfers
variables of type {\it intvar} into type {\it term} }.}
Finally the branching strategy branches over the vector {\it letters}.\\
...
\caption{{\bf Search-tree of the Safe Puzzle}}
\label{figure9}
\end{figure}
\begin{figure}[htpb]
...
used - {\it constrain}. It demands for further explanation. 
Listing \ref{listing 3} shows an example how defined constraints are
realized. A new datatype - {\it constraint} - to modularize propagators 
is declared. Here, a constraint
can be a sum, a product or a '$<$' - relation. 
The \db{procedure} {\it post(space,SUMV(xs) `= `c,FD.BND)} creates a propagator that 
...
you need a temporary variable for every product of two variables because {\it FD.mult}
only can propagate x * y = z. Both procedures use predefined propagators from the
\htmladdnormallink{\textcolor{blue}{FD}}
{http://www.ps.uni-sb.de/alice/manual/library/fd.html}
library.}

\lstset{caption = {\bf Defined Constraints},label=listing 3,frame=trbl}
\begin{myverbatim}
...
\subsubsection{Script}
\lstset{caption = {\bf A script for the Grocery Puzzle},
...
\lstset{caption = {\bf Script for the Family Puzzle},
...
\lstset{caption = {\bf Script for the Zebra Puzzle},
...
\lstset{caption = {\bf Script for the N-queens Problem},
...
$ 0 \leq i \le j \le N $
\end{quote}  
This list is given as parameter to the {\it List.app} \db{procedure} and
...
\lstset{caption = {\bf Help functions for the N-queens Problem},
...
The vectors {\it add} and {\it sub} are used to build the constraints
...
\lstset{caption = {\bf Optimization of the script for the N-queens Problem},
...
\lstset{caption = {\bf A script for the Changing Money
...
\lstset{caption = {\bf A script for Sudoku Puzzle},
...
have the same color.\\
\lstset{caption = {\bf A script for the Map Coloring Problem},
...
\lstset{caption = {\bf A script for conference scheduling},
...
\subsubsection{Script}
\lstset{caption = {\bf Script for the Fractions Problem},
...
\subsubsection{Script}
\lstset{caption = {\bf A script that enumerates Pythagoras triples},
...
\db{use the {\it post} - modelling tool}. Then you would not need to write:
\begin{quote}
 val aa = FD.range (space, (0, maxValue))\\
 val bb = FD.range (space, (0, maxValue))\\
 val cc = FD.range (space, (0, maxValue))\\
...
\subsubsection{Script}
\lstset{caption = {\bf Script for the Magic Square Puzzle},
...
\subsubsection{Script}
\lstset{caption = {\bf A script for the Photo Puzzle},
...
\subsubsection{Script}
\lstset{caption = {\bf A script for the self-referential aptitude test},
...
\lstset{caption = {\bf A script for the Bin Packing Problem},
...
\subsubsection{Script}
\lstset{caption = {\bf The revised script for the Photo Puzzle},
...
\lstset{caption = {\bf ordering procedure},label=listing 21,frame=trbl}
\begin{myverbatim}
fun photoorder(current,last)=
               post (current, FD(satisfaction)`> 
            `(FD.Reflect.value(last,satisfaction)),FD.BND)
...
\lstset{caption = {\bf type of revised photo script},
...
\subsubsection{Script}
\lstset{caption = {\bf A script for the Send Most Money Puzzle},
...
\subsubsection{Script}
\lstset{caption = {\bf A script for the warehouse problem},
...
\lstset{caption = {\bf A script for the Steiner Problem},
...
\end{quote}
\lstset{caption = {\bf Extension of the Steiner Script},
...
($\#$(${\it s_a} \cap {\it s_b}$)) \db{resp. is not contained 
($\#$(\{1,$\ldots$,{\it b}$\backslash {\it s_a} \cup {\it s_b}$) 
in both sets. Thus, the Hamming distance results in 
\begin{quote}
{\it h(a, b)} = {\it $s_b$} - {\it $\#$ ($s_a \cap s_b $)}
 - $\#$(\{1,$\ldots$,{\it b}$\backslash {\it s_a} \cup {\it s_b}$).  
\end{quote}}

\subsubsection{Script}
\lstset{caption = {\bf A Script for the Hamming Codes Problem},
...
branching strategy because the value of {\it nbDisks} would have to be
{\it reflected} during search}.

\subsubsection{Script}
\lstset{caption = {\bf A Script for the Packing Files Problem},
...
\lstset{caption = {\bf Test function for the Packing 
Files Problem},
...
\end{itemize}
\lstset{caption = {\bf A Script for the Crew Allocation Problem},
...
\ref{table3}(using FS.cardinality and {\it post})}. \\
...
\lstset{caption = {\bf Sample data for the Crew Allocation Problem},
...
\lstset{caption = {\bf A Script for the Golf Problem},
...
\lstset{caption = {\bf Sample data for the Golf Problem},
...
numbers positions. 
\lstset{caption = {\bf A Script for the Sudoku Problem},
...
\subsubsection{Script}
\lstset{caption = {\bf A combined Script for the Sudoku Problem},
...
\bibitem[1]{1} {\it Finite Domain Constraint Programming in Oz. A Tutorial.} by \\ 
{\it Christian Schulte and Gert Smolka}

\bibitem[2]{2} {\it Problem Solving with Finite Set Constraints in Oz. A Tutorial.} by \\ 
{\it Tobias M\"uller}

\bibitem[3]{3} {\it Principles of Constraint Programming.} by \\
 {\it Krysztof R. Apt.}, Cambridge University Press, 2003.

\bibitem[4]{4} {\it Short Tour to Alice} by\\
{\it Andreas Rossberg}, http://www.ps.uni-sb.de/alice/manual/tour.html

\bibitem[5]{5} {\it Modelling for Constraint Programming} by\\
{\it Barbara M. Smith}, in First International Summer School
on Constraint Programming, September 2005

\bibitem[6]{6} {\it Dual Viewpoint Heuristics for Binary
...
\bibitem[8]{8}{\it Sudoku as a Constraint Problem} by\\
...
\bibitem[11]{11} {\it Topics on steiner systems} by \\ 
