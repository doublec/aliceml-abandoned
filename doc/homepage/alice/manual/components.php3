...
<P>Components can be created dynamically as first-class values, by means of the
<A href="library/component.php3"><TT>Component</TT></A> structure. This way,
components may be stored or passed to other processes. A first-class component
may be written to a file and can then be imported in the usual way using <A
href="#source">import announcements</A>. Inversely, component files generated
by the compiler can be read as first-class components.</P>

<P>First-class components can be evaluated using the <A
href="library/component-manager.php3#Eval-fn"><TT>ComponentManager.Eval</TT></A>
functor. The result is the component's export module.</P>
...
restricted to ground signatures: their export may be higher-order. Components
...
href="packages.php3#dynamic">generative</A> entities, like types or
exceptions.<P>

<P>Evaluated components stored to a file are called <A
href="pickling.php3"><EM>pickles</EM></A>. A pickle is a serialized and closed
representation of a value. The library structure <A
href="library/pickle.php3"><TT>Pickle</TT></A> provides convenient operations
...
<?php section("managers", "component managers") ?>

<P>An application's root component is located and loaded by the root <A
href="library/component-manager.php3">component manager</A>. An application may
create custom managers using the <A
href="library/component.php3#MkManager"><TT>Component.MkManager</TT></A>
functor. By evaluating a <A href="#firstclass">first-class component</A> using
the <A href="library/component-manager.php3#Eval-fn"><TT>Eval</TT></A> functor
of a custom manager, all direct and indirect imports of that component will be
handled by that respective manager. This particularly allows setting up custom
<EM>sandbox environments</EM> for untrusted components.</P>
