structure Table :> TABLE =
	...
	fun makeTable re =
		...
		and checktrans state ord =
			 ...
			 *)
			val state' = IntSet.foldl check IntSet.empty state
		    in
	...
	fun minTable (finstates, dtran) = 
		...
		val part =
			...
			val p' = IntMap.listItems p 
		    in
			map (fn x => IntSet.listItems x) p'
		    end
	...
	and partition (finstates, dtran) =
		...
		val groupNum = ref 0
		val actGroup = Array.array(IntMap.numItems finstates + 1, ~1)
		...
		fun startpart finstates =
		    let
			fun oldSet group = 
			    case IntMap.find( !partMap, group) of
				NONE     => IntSet.empty
			      | SOME set => set
		...
		and split' (group, is) =
			...
			val head = hd is
			val tail = tl is
			val comp = Vector.tabulate (257, (getGroup head) )
			val (equal, diff) = compare (comp, tail, [head], nil)
		    in
			if diff = nil then ()
			else (changed := true;
			      app
		...
		and getGroup state ord =
		    let
			val p =
			    case IntMap.find(dtran, state) of
				SOME x => x
	...
	fun lineUp (finstates, dtran) =
		...
		val Finstates = ref (IntMap.listItemsi finstates)
		val Dtran = ref (IntMap.listItemsi dtran)
