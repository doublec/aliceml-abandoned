...
public:
  ...
  static u_int EncodeHeader(BlockLabel l, u_int s, u_int gen,
			    u_int mutableFlag=0) {
    AssertStore(static_cast<u_int>(l) <= MAX_STORE_LABEL);
    ...
    if (s > MAX_BLOCKSIZE) {
      ...
      return (((gen + 1) << GEN_GC_SHIFT) |
	      ...
	      (static_cast<u_int>(l) << TAG_SHIFT |
	       mutableFlag << MUTABLE_SHIFT));
    }
    else {
      return (((gen + 1) << GEN_GC_SHIFT) | (s << SIZE_SHIFT) |
	      (static_cast<u_int>(l) << TAG_SHIFT | mutableFlag << MUTABLE_SHIFT));
    }
  }
  static u_int GetHeader(Block *p) {
    AssertStore(p != INVALID_POINTER);
    return reinterpret_cast<u_int *>(p)[0];
  }
  // Label Creation and Access
  static void EncodeLabel(Transient *p, BlockLabel l) {
    AssertStore(p != INVALID_POINTER);
    AssertStore(static_cast<u_int>(l) <= MAX_STORE_LABEL);
    reinterpret_cast<u_int *>(p)[0] =
      ((reinterpret_cast<u_int *>(p)[0] & ~TAG_MASK) | (static_cast<u_int>(l) << TAG_SHIFT));
  }
  static BlockLabel DecodeLabel(Block *p) {
    AssertStore(p != INVALID_POINTER);
    return static_cast<BlockLabel>((reinterpret_cast<u_int *>(p)[0] & TAG_MASK) >> TAG_SHIFT);
  }
  static u_int DecodeMutableFlag(Block *p) {
    AssertStore(p != INVALID_POINTER);
    return static_cast<u_int>((reinterpret_cast<u_int *>(p)[0] & MUTABLE_MASK) >> MUTABLE_SHIFT);
  }
  static void EncodeMutableFlag(Block *p, u_int f) {
    AssertStore(p != INVALID_POINTER);
    AssertStore(f<=1);
    reinterpret_cast<u_int *>(p)[0] =
      ((reinterpret_cast<u_int *>(p)[0] & ~MUTABLE_MASK) | (f << MUTABLE_SHIFT));
  }
  // Size Creation and Access
  static void EncodeSize(Block *p, u_int s) {
    AssertStore(p != INVALID_POINTER);
    AssertStore(s <= MAX_BLOCKSIZE);
    reinterpret_cast<u_int *>(p)[0] = ((reinterpret_cast<u_int *>(p)[0] & ~SIZE_MASK) | (s << SIZE_SHIFT));
  }
  static u_int DecodeSize(Block *p) {
    AssertStore(p != INVALID_POINTER);
    u_int h = reinterpret_cast<u_int *>(p)[0];
    return static_cast<u_int>(((h & SIZE_MASK) >> SIZE_SHIFT) << (h & SIZESHIFT_MASK));
  }
  // Generation Access
  static u_int DecodeGeneration(Block *p) {
    AssertStore(p != INVALID_POINTER);
    return ((reinterpret_cast<u_int *>(p)[0] & GEN_GC_MASK) >> GEN_GC_SHIFT) - 1;
  }
  // Intgen Mark Access
  static void SetChildishFlag(Block *p) {
    AssertStore(p != INVALID_POINTER);
    reinterpret_cast<u_int *>(p)[0] |= (static_cast<u_int>(1) << CHILDISH_SHIFT);
  }
  ...
  static u_int IsChildish(Block *p) {
    AssertStore(p != INVALID_POINTER);
    return reinterpret_cast<u_int *>(p)[0] & CHILDISH_MASK;
  }
