...
structure PickleOutStream :> PICKLE_OUT_STREAM =
	...
	type outstream =
	     {array: array ref,
	      index: int ref,
	      label: LargeWord.word ref,
	      crc: Crc.t ref,
	      count: int ref}
	type label = word

	fun outputByte ({array, index, label, crc, count}, b) =
	...
	fun inc ({array, index, label, crc, count}, i) =
	    count := Int.+ (!count, i)

	fun inc1 q = inc (q, 1)

	fun dec1 {array, index, label, crc, count} =
	    if !count = 0 then
		raise Crash.Crash "PickleOutStream.dec1"
	    else count := Int.- (!count, 1)

	fun newLabel {array, index, label, crc, count} =
	    let
		val n = !label
	    in
		label := n + 0w1; n
	...
	and outputInt (q, i) = outputLargeWord (q, fromInt i)
	and outputLargeInt (q, i) = outputLargeWord (q, fromLargeInt i)

	and marshalSmallInt (q, i) =
	    (outputByte (q, SMALLINT); marshalSmallInt' (q, i))
	and marshalSmallInt' (q, i) =
	    if i >= 0wx80 then
		(outputByte (q, Word8.fromLargeWord (i mod 0wx80 + 0wx80));
		 marshalSmallInt' (q, i div 0wx80))
	...
	and marshalString (q, s) =
	    (outputLargeWord (q, fromInt (String.size s));
	     CharVector.app
	...
	fun outputLargeReal (q, r) =
		...
		outputLargeWord (q, i1); outputLargeWord (q, i2)
	    end
	...
	fun outputAtom (q, s) =
	    ...
	    in
		dec1 q; outputByte (q, ATOM); outputLargeWord (q, label);
	...
	and marshalUniqueName (q, s) =
		...
		val label = newLabel q
	    in
		dec1 q; outputByte (q, UNIQUENAME); outputLargeWord (q, label);
		marshalString (q, s); label
	    end
	...
	fun outputTuple (q, lab, width) =
		...
		val label = newLabel q
	    in
		dec1 q; outputByte (q, TUPLE); outputLargeWord (q, label);
		inc1 q; outputInt (q, width); inc1 q; outputAtom (q, lab);
		inc (q, width); label
	    end

	fun outputList _ (q, nil) = (dec1 q; outputAtom (q, "nil"))
	  | outputList outputX (q, x::xs) =
	    let
		val label = newLabel q
	    in
		dec1 q; outputByte (q, LIST); outputLargeWord (q, label);
		inc1 q; outputX (q, x); marshalList (q, xs, outputX); label
	    end
	and marshalList (q, x::xs, outputX) =
	    (outputByte (q, LIST); outputLargeWord (q, newLabel q);
	     inc1 q; outputX (q, x); marshalList (q, xs, outputX))
	  | marshalList (q, nil, _) =
	    (outputByte (q, LIST_END); inc1 q; outputAtom (q, "nil"))
	...
	fun outputRef outputX (q, ref x) =
		...
		val label = newLabel q
	    in
		dec1 q; outputByte (q, CELL); outputLargeWord (q, label);
		inc1 q; outputX (q, x); label
	    end

	fun outputShared (q, label) =
	    (dec1 q; outputByte (q, REF); outputLargeWord (q, label))
	...
	fun openOut (): outstream =
		...
		outputByte (q, 0w0); outputByte (q, 0w0); outputByte (q, 0w0);
		inc1 q; outputString (q, versionString);
		q
	    end

	fun closeOut (q as {array, index, label, crc, count}) =
	    let
