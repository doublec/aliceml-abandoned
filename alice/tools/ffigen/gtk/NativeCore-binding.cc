...
DEFINE1(NativeCore_isNull) {
  DECLARE_UNMANAGED_POINTER(y0,x0);
  RETURN(BOOL_TO_WORD((int)(y0 == NULL)));
} END
...
DEFINE1(NativeCore_hasSignals) {
  DECLARE_OBJECT_WITH_TYPE(obj,type,x0);
  RETURN(BOOL_TO_WORD(false));//BOOL_TO_WORD(type == TYPE_GTK_OBJECT));
} END
...
word InitComponent() {
  ...
  INIT_STRUCTURE(record, "NativeCore", "isNull", 
		 NativeCore_isNull, 1);
