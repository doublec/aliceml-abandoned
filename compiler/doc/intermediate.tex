...
Type names $c$ carry an openness flag which is true only for open sum types. First-class constructors for an open sum type $t$ are assigned constructor arrow types $t' \conarrow t$ where $t'$ is the argument type and $t = c(t_1)\cdots(t_n)$ for some $n\geq0$.
...
\begin{equation}
\frac{
E \vdash \lab \Rightarrow a
\qquad
E \vdash \longid \Rightarrow \{a:t,r\}
}{
...
E \vdash \f{abs}\;\exp : t \Rightarrow t
}
\end{equation}
...
E \vdash \f{unabs}\;\exp : t \Rightarrow t
}
\end{equation}
...
\frac{
\begin{array}{@{}c@{}}
E \vdash \fld^x_i \Rightarrow a_i,t_i\langle,E_i\rangle
\qquad
\mbox{$a_i$ disjoint}
\\
r = a_1:t_1,\cdots,a_n:t_n,\epsilon
\qquad
...
\fbox{$E \vdash \fld^x \Rightarrow a,t\langle,E'\rangle$}
}
...
E \vdash \lab = x \Rightarrow a,t\langle,E'\rangle
}
\end{equation}
